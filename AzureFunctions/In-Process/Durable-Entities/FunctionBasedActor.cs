...
{
    public static class FunctionBasedActor
    {
        [FunctionName("FunctionBasedActor")]
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = "functionbasedactor/{entityId}/{operation}")] HttpRequest req,
            ...
            ILogger log, string entityId, string operation)
        {
            ...
            {
                case "add":
                    await entityClient.SignalEntityAsync(entityIdObj, operation, inputValue);
                    break;
                case "reset":
                    await entityClient.SignalEntityAsync(entityIdObj, operation);
                    break;
                case "delete":
                    await entityClient.SignalEntityAsync(entityIdObj, operation);
                    break;
